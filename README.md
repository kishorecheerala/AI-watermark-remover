...
Remove **visible** and **invisible** AI watermarks from images and videos generated by Google Gemini (Nano Banana), ChatGPT / DALL-E, Stable Diffusion, Adobe Firefly, Midjourney, Kling, Doubao, Samsung Galaxy AI, and other AI models.

[![Download Android APK](https://img.shields.io/badge/📱%20Direct%20Download-AI--Watermark--Remover--Studio.apk-success?style=for-the-badge&logo=android)](https://github.com/kishorecheerala/AI-watermark-remover/raw/main/AI-Watermark-Remover-Studio.apk)
[![Launch GUI Studio](https://img.shields.io/badge/🎨%201--Click%20GUI-Launch%20Studio-blue?style=for-the-badge&logo=apple)](https://github.com/kishorecheerala/AI-watermark-remover/blob/main/Launch%20AI%20Studio.command)

> ### 📱 Direct APK Download: [Download AI-Watermark-Remover-Studio.apk](https://github.com/kishorecheerala/AI-watermark-remover/raw/main/AI-Watermark-Remover-Studio.apk)
> Download and install the standalone mobile Android APK directly on your phone or Samsung Galaxy device for instant media watermark removal.
>


> ## 🎨 Interactive GUI Studio (No CLI Required)
>
> Double-click **`Launch AI Studio.command`** (macOS) or **`Start AI Studio.bat`** (Windows) to automatically start the local Studio server and open the interactive web app interface in your browser window.
>
...
- **Interactive 1-Click GUI Studio** — Double-click `Launch AI Studio.command` (macOS) or `Start AI Studio.bat` (Windows) to run the full offline Studio interface in your browser window with interactive canvas, magic wand region selector, live FFT steganography inspector, and split slider.
- **Video AI Watermark Removal (`video`)** — Frame-by-frame visible watermark inpainting with optical flow anti-flicker mask smoothing across consecutive frames for AI video generators (Sora, Kling 3.0, Runway Gen-3, Luma, MiniMax, Samsung Galaxy AI video watermarks). Re-stitches original audio tracks losslessly via `ffmpeg`.
- **Audio AI Watermark Removal & Artifact Suppression (`audio`)** — Spectral phase perturbation and adaptive notch filtering to neutralize invisible audio watermarks (ElevenLabs, Suno, Udio, Meta AudioSeal) alongside power-hum (< 60Hz) and comb-filter artifact reduction.
- **Smart Filename Sanitizer & EXIF Humanizer** — Automatically transforms AI-suggesting filenames (`ChatGPT_Image_*`, `Gemini_Generated_*`, `DALL·E_*`, `SDXL_*`, `Kling_*`, `Jimeng_*`) into clean camera roll patterns (`20260726_083908.jpg` / `DSC_4829.jpg`). Optionally synthesizes non-AI smartphone / DSLR EXIF metadata headers.
- **Samsung One UI & Galaxy Integration** — Mobile integration providing Samsung Share Sheet support, Quick Settings tile ("Clean AI Mark"), S-Pen Smart Select crop capture, and 1-tap hand-off into Samsung's native Video Editor (`com.samsung.android.videoeditor`). Outputs named APK binary `AI-Watermark-Remover-Studio.apk`.
- **Manifest V3 Browser Extension** — Browser extension in `extension/` providing right-click *"Clean AI Watermark & Download"* context menu action for web photos and videos.
- **Visible watermark removal** — a registry of known marks in their usual places: the Gemini / Nano Banana sparkle, the Doubao "豆包AI生成" text strip, the Jimeng "★ 即梦AI" wordmark, the Qwen "千问AI生成" and Kling "可灵AI 3.0" text strips, the Baidu "百度 AI生成" text + tag, the LibLibAI wordmark (bottom-center), the RunningHub "RunningHub AI生成" text (top-left), and the Samsung Galaxy AI strip (bottom-left, multi-lingual). Each mark is **localized to a footprint mask, then filled**.
...
- **Detection & Provenance Identification (`identify`)** — aggregate C2PA issuer, forensic watermark vendor, IPTC, embedded params, EXIF/XMP tags, Grok signature, China TC260 AIGC labels, visible marks, and invisible watermarks into one unified verdict.
